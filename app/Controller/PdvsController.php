 ...
 */
  ...
  public function todosPDVs($id = 0){
            return $this->Pdv->find('all',
            	array('conditions'=>array('Pdv.IdAcao'=>$id)));

  }   

  public function retornaProdutos($id = 0){
  	return $this->Pdv->find ('all', array(
  		'fields'=> array('DISTINCT Pdv.Produto'),
  		'conditions'=>array('Pdv.IdAcao'=>$id)
  	));
  }

public function retornaPromotores($id = 0){
  	return $this->Pdv->find ('all', array(
  		'fields'=> array('DISTINCT Pdv.Promotor'),
  		'conditions'=>array('Pdv.IdAcao'=>$id)
  	));
  }

public function retornaPdvPromotores($id = 0, $promotor = null){
	return $this->Pdv->find ('all', array('conditions'=>array('Pdv.IdAcao'=>$id, 'Pdv.Promotor'=>$promotor)));
}
public function retornaQuantidadePdv($idAcao=0){
  return $this->Pdv->find('count', array('conditions'=>array('Pdv.IdAcao'=>$idAcao)
));
}
