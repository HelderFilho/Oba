<?php
    ...
    $fpdf->Cell(0, 12, utf8_decode('Relatório de Ações'),0,0,'C', true);
    $fpdf->Ln();
    $fpdf->Ln();
    ...
    $contL = 0;
    $altura = 10;
    $Reponsavel = $this->requestAction('Clientes/retornaResponsavel/'.$idCliente);
  ...
  if ($user['empresa']=='Top'){
    ...
    $fpdf->Cell(0,10,'Dados',1,0,'C',true);
    $fpdf->Ln();
       $fpdf->SetTextColor(0);
$fpdf->SetFont('','B',8);
    $fpdf->Cell(25, 7, 'Nome',1,0, 'C', false);
    $fpdf->Cell(43, 7, utf8_decode('Endereço'),1,0,'C', false);
    $fpdf->Cell(20, 7, 'Email', 1, 0, 'C', false);
    $fpdf->Cell(15, 7, 'Telefone', 1,0, 'C', false);
    $fpdf->Cell(20, 7, 'Quantidade', 1,0, 'C', false);
    $fpdf->Cell(15, 7, 'Cupom', 1, 0,'C', false);
    $fpdf->Cell(12, 7, 'Valor', 1, 0,'C', false);
    $fpdf->Cell(20, 7, 'Loja', 1, 0, 'C', false);
    $fpdf->Cell(20, 7, 'Nascimento', 1, 0, 'C', false);
    $fpdf->Ln();
    $CRM = $this->requestAction('/Crms/RetornaCRM/' . $acao['Acao']['Id']);
          $fpdf->SetFont('','',7);

    foreach ($CRM as $cadastro) {
    $fpdf->Cell(25, 7, utf8_decode($cadastro['Crm']['Nome']),1,0, 'C', false);
    $fpdf->Cell(43, 7, utf8_decode($cadastro['Crm']['Endereco']),1,0,'C', false);
    $fpdf->Cell(20, 7, utf8_decode($cadastro['Crm']['Email']), 1, 0, 'C', false);
    $fpdf->Cell(15, 7, utf8_decode($cadastro['Crm']['Telefone']), 1,0, 'C', false);
    $fpdf->Cell(20, 7, utf8_decode($cadastro['Crm']['Quantidade']), 1,0, 'C', false);
    $fpdf->Cell(15, 7, utf8_decode($cadastro['Crm']['Cupom']), 1, 0,'C', false);
    $fpdf->Cell(12, 7, utf8_decode($cadastro['Crm']['Valor']), 1, 0,'C', false);
    $fpdf->Cell(20, 7, utf8_decode($cadastro['Crm']['Loja']), 1, 0, 'C', false);
    $Nascimento = substr($cadastro['Crm']['Nascimento'], 0,10);
    $fpdf->Cell(20, 7, utf8_decode($Nascimento), 1, 0, 'C', false);
     $fpdf->Ln(); 
